"""
Voter List PDF Parser.
Extracts structured voter data from Indian Election Commission Voter List PDFs.
Handles embedded custom Devanagari fonts (Rajasthan ECI PUA encoding),
extracts Part No (Bhag Sankhya), exact Serial No (Kram Sankhya) directly from card boxes,
Names, Relation, Age, Gender, House No, EPIC, and transliterates to English.
Preserves exact printed serial numbers without any shifting or skipping.
"""
...
def apply_reph_to_chars(chars: List[str]):
    """
    In Devanagari Unicode, reph 'र्' (RA + VIRAMA) must precede the base consonant
    it sits on top of. Walks backwards past trailing vowel matras to position 'र्'.
    """
    idx = len(chars) - 1
...
def extract_part_number(doc: pymupdf.Document, filepath: str) -> str:
    """
    Extracts Part Number (Bhag Sankhya) from PDF metadata, filename, or page 1/3 text.
    """
    filename = os.path.basename(filepath)
    part_m = re.search(r"Part\s*(?:No)?[-_\s]*0*([0-9]+)", filename, re.IGNORECASE)
    if part_m:
        return str(int(part_m.group(1)))

    if len(doc) > 0:
        p1_text = doc[0].get_text()
        m = re.search(r"(?:भाग|मरग)\s*(?:संख्या|सगखजर)\s*[:\-]\s*([0-9]+)", p1_text)
        if m:
            return m.group(1)
    ...
    if len(doc) > 2:
        ...
        if xobjs:
            ...
            m = re.search(r"K!\+\s*98G#!\s*:\s*([0-9]+)", s)
            if m:
                return m.group(1)
...
def parse_voter_card(voter_bt: str, serial_num: int) -> dict:
    """
    Parses an individual voter card's BT text stream.
    """
    # 1. EPIC: matches standard 10-char alphanumeric IDs or older slash formats (e.g. RJ/11/085/399175)
...
def parse_page_voters(doc: pymupdf.Document, pno: int, bhag_sankhya: str) -> List[Dict[str, Any]]:
    """
    Extracts all voter records on a specific PDF page using geometric / cell alignment.
    Pairs each voter card with its EXACT printed serial number (Kramank).
    """
    page = doc[pno]
    ...
    # Match each voter block to its exact serial block via spatial alignment
    # In this PDF layout: voter_x ≈ serial_x + 13.75, voter_y ≈ serial_y - 53.05
    page_voters = []
    for vx, vy, vbt in voter_blocks:
        ...
        for sx, sy, snum, sbt in serial_blocks:
            ...
            if dist < best_dist and dist < 40.0:
                best_dist = dist
                matched_serial = snum
        ...
        if matched_serial is None:
            # Direct serial pattern inside voter BT block
...
def parse_voter_pdf(filepath: str) -> Dict[str, Any]:
    """
    Parses an entire voter list PDF into structured voter records.
    Deduplicates by Kramank number and ensures exact serial numbers.
    """
    ...
    filename = os.path.basename(filepath)
    bhag_sankhya = extract_part_number(doc, filepath)

    voters_by_kram = {}
    start_page = 2 if len(doc) > 2 else 0

    for pno in range(start_page, len(doc)):
        page_voters = parse_page_voters(doc, pno, bhag_sankhya)
        for v in page_voters:
            v["pdf_filename"] = filename
            k = v["kram_sankhya"]
            if k not in voters_by_kram:
                voters_by_kram[k] = v
            else:
                # If either entry indicates deleted, preserve deletion status
                if v.get("is_deleted"):
                    voters_by_kram[k]["is_deleted"] = True
                # If existing entry had shorter/weaker names, update with better one
                if len(v.get("name_hindi", "")) > len(voters_by_kram[k].get("name_hindi", "")):
                    voters_by_kram[k]["name_hindi"] = v["name_hindi"]
                    voters_by_kram[k]["name_english"] = v["name_english"]

    doc.close()

    unique_voters = list(voters_by_kram.values())
    unique_voters.sort(key=lambda x: x["kram_sankhya"])

    return {
        "filename": filename,
        "bhag_sankhya": bhag_sankhya,
